...
export class UserDetailsService {
  ...
  deleteP(productId : Number): Observable<any>{
    let url = "http://localhost:8084/finance-II/api/product-delete?productId=" +productId;
    return this.http.delete(url);
  }
  ...
  editP(product: Product) : Observable<any> {
    let url = "http://localhost:8084/finance-II/api/product-update";
    return this.http.post(url,product);
  }


  saveP(product: Product) : Observable<any> {
      let url = "http://localhost:8084/finance-II/api/save-product";
      return this.http.post(url,product);
  }


  addCard(userId : Number, cardType : String) :  Observable<any> {
    let url = "http://localhost:8084/finance-II/api/addCard?userId=" +userId+"&cardType=" + cardType;
    return this.http.get(url);
  }
}